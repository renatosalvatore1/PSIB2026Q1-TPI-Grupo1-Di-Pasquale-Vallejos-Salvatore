...
'''
 forma correcta usa coordenadas MNI reales y registro al espacio estándar:
 (ver como hacer esto, lo que esta ahora es solo de prueba)
 # 1. Registrar la imagen al espacio MNI152 (requiere ANTs o FSL)
#    Esto "deforma" el cerebro del paciente al cerebro estándar

# 2. Usar coordenadas MNI reales de la literatura
ATLAS_MNI = {
    "Área de Broca":           (-45, 20, 10),   # coordenadas x,y,z en mm
    "Área de Wernicke":        (-55, -40, 15),
    "Corteza Motora Primaria": (-30, -20, 60),
    "Hipocampo izq":           (-25, -20, -15),
    "Tronco Encefálico":       (0,   -30, -35),
}

# 3. Convertir vóxel → mm usando la matriz affine
def voxel_to_mni(voxel_coord, affine):
    vox = np.array([*voxel_coord, 1])
    mni = affine @ vox
    return mni[:3]  # x, y, z en mm reales
'''
...
class MRITumorAnalyzer:

    def __init__(self, filephat:str):
        self.filepath = filephat
        self.filename = os.path.basename(filephat)
        print(f"\n{'═'*60}")
        print(f"  Cargando: {self.filename}")
        print(f"{'═'*60}")

        img = nib.load(self.filepath)
        self.data_raw = img.get_fdata()
    ...
    def _preprocess(self) -> np.ndarray:
        ...
        1.Normalizacion robusta ( percentil)
        2. Eliminacion de craneo ( skull stripping simplificado por umbral)
        3. Filtro Wiener por corte ( reduccion de ruido + modelo AR implicito)
        4. Ecualizacion adaptatica del histograma
        '''
        ...
        # Filtro Wiener corte a corte (modelo AR 2D → reduce ruido manteniendo bordes)
        vol_filt = np.zeros_like(vol)
        for z in range(vol.shape[2]):
            sl = vol[:, :, z]
            # Wiener solo donde hay tejido
            if sl.max() > 0.01:
                vol_filt[:, :, z] = wiener(sl, mysize=5)
            else:
                vol_filt[:, :, z] = sl

        # CLAHE por corte (mejora contraste local)
        vol_clahe = np.zeros_like(vol_filt)
        for z in range(vol.shape[2]):
            sl = vol_filt[:, :, z]
            if sl.max() > sl.min():
                vol_clahe[:, :, z] = exposure.equalize_adapthist(
                    np.clip(sl, 0, 1), clip_limit=0.03
                )
            else:
                vol_clahe[:, :, z] = sl

        # Aplicar máscara cerebral
        vol_clahe *= brain_mask.astype(float)
...
def _detect_tumor(self) -> np.ndarray:
    ...
    thresh_high = np.percentile(brain_vals, 87)
...
def show(self):
    ...
    tumor_cmap = ListedColormap([(0, 0, 0, 0), (1, 0.1, 0.1, 0.6)])
    ...
    im_ax   = ax_img.imshow(sl.T, cmap='gray', origin='lower', aspect='equal')
    ov_ax   = ax_img.imshow(tm.T, cmap=tumor_cmap, origin='lower', aspect='equal',
...
def main():
    ...
    # ── Análisis ─────────────────────────────────────────────
    try:
        analyzer = MRITumorAnalyzer(filepath)
        analyzer.show()
